...
export class ProjectExistsGuard implements CanActivate {
  ...
  public canActivate(route: ActivatedRouteSnapshot): Observable<boolean> {
    return this.hasEntity(route.params['ProjectId'] || route.params['id']);
  }

  protected hasEntityInStore(id: string): Observable<boolean> {
    return this.store.select(getProjectById(id)).pipe(
      map((entitie) => !!entitie),
      take(1)
    );
  }
  ...
  protected hasEntityInApi(id: string): Observable<boolean> {
    return this.Project.exists(id).pipe(
      map((response: any) => !!response.exists),
      catchError(() => {
        this.store.dispatch(new ProjectActions.guardFail());
        return of(false);
      })
    );
  }
